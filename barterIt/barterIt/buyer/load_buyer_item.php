...
if (isset($_POST['userid'])){
	...
	if($option == 1){
	    ...
	    $sqlloaditems = "SELECT * FROM `tbl_items` WHERE item_type LIKE '%$item_type%'";
	    $sqlcart = "SELECT * FROM `tbl_carts` WHERE user_id = '$cartuserid'";
	}
}else{
...
if ($result->num_rows > 0) {
    ...
    $response = array('status' => 'success', 'data' => $items, 'numofpage'=>"$number_of_page",'numberofresult'=>"$number_of_result", 'cartqty'=> $totalcart, 'numberofcartitem'=> $number_of_result_cart);
    sendJsonResponse($response);
}else{
     $response = array('status' => 'failed', 'data' => null);
    sendJsonResponse($response);
