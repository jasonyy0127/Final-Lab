...
$signed= hash_hmac('sha256', $signing, 'S-Q5ov-l3jj-bEuUcsahuGSg');
    ...
    if ($paidstatus == "Success"){ //payment success
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $sqlorderdetails = "INSERT INTO `tbl_orderdetails`(`order_bill`, `item_id`, `orderdetail_qty`, `orderdetail_paid`, `buyer_id`, `seller_id`, `orderdetail_date`) VALUES ('$receiptid','$itemid','$orderqty','$order_paid','$userid','$seller_id','$date')";
                $conn->query($sqlorderdetails);
                $sqlupdateitemqty = "UPDATE `tbl_items` SET `item_quantity`= (item_quantity - $orderqty) WHERE `item_id` = '$itemid'";
